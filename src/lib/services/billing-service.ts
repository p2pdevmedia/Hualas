...
export const billingService = {
  generateMonthlyChargesForMember: async (
    memberId: string,
    month: number,
    year: number,
    amount = 0
    ...
    prisma.memberMonthlyCharge.upsert({
      where: {
        memberId_periodMonth_periodYear_concept: {
          memberId,
          periodMonth: month,
          periodYear: year,
  ...
  getPendingChargesForFamily: async (
    ...
    prisma.memberMonthlyCharge.findMany({
      where: {
        ...
        periodYear: year,
        status: 'PENDING',
        member: { familyMemberships: { some: { familyGroupId } } },
      },
    }),
  markMemberMonthlyChargeAsPaid: (id: string, paymentId: string) =>
    prisma.memberMonthlyCharge.update({
      where: { id },
