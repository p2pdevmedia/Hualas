...
const concepts = [
  ...
  { code: BillableConceptCode.DISCOUNT, name: 'Descuento' },
...
async function main() {
  for (const concept of concepts) {
    await prisma.billableConcept.upsert({ where: { code: concept.code }, create: concept, update: { name: concept.name, active: true } });
  }
}
