...
import { POST as postPickupNotice, GET as getPickupNotices } from "@/app/api/activity-days/[dayId]/pickup-notices/route";
import { PUT as putNotice, DELETE as deleteNotice } from "@/app/api/pickup-notices/[noticeId]/route";
...
function makePostRequest(body: object) {
  return new Request(`http://localhost/api/activity-days/${DAY_ID}/pickup-notices`, {
    method: "POST",
...
describe("POST /api/activity-days/[dayId]/pickup-notices", () => {
  it("returns 401 when there is no session", async () => {
    ...
    const res = await postPickupNotice(
      makePostRequest({ childId: CHILD_ID, alternatePersonName: "Tía María", description: "Test" }),
      makeDayContext()
    );
  ...
  it("returns 404 when activity day is not found", async () => {
    (getServerSession as jest.Mock).mockResolvedValueOnce({ user: SESSION_USER });
  ...
  it("returns 400 when an active notice already exists for the same child+day", async () => {
    ...
    mockPrisma.activityParticipant.findFirst.mockResolvedValueOnce({ id: "part_001" });
  ...
  it("creates a pickup notice when all validations pass", async () => {
    ...
    const res = await postPickupNotice(
      makePostRequest({ childId: CHILD_ID, alternatePersonName: "Tía María", description: "Sister will pick up" }),
      makeDayContext()
    );
  ...
  it("restores a soft-deleted notice instead of creating a duplicate", async () => {
    ...
    const res = await postPickupNotice(
      makePostRequest({ childId: CHILD_ID, alternatePersonName: "Tía María", description: "Restored" }),
      makeDayContext()
    );
...
describe("GET /api/activity-days/[dayId]/pickup-notices", () => {
  it("returns 401 when not authenticated", async () => {
    ...
    const req = new Request(`http://localhost/api/activity-days/${DAY_ID}/pickup-notices`);
  ...
  it("returns 404 when activity day not found", async () => {
    (getServerSession as jest.Mock).mockResolvedValueOnce({ user: PROFESSOR_USER });
    mockPrisma.activityDay.findUnique.mockResolvedValueOnce(null);
  ...
  it("returns 403 when professor is not assigned to the day", async () => {
    (getServerSession as jest.Mock).mockResolvedValueOnce({ user: PROFESSOR_USER });
    mockPrisma.activityDay.findUnique.mockResolvedValueOnce({ id: DAY_ID, date: FUTURE_DATE });
  ...
  it("returns 200 with notices for an assigned professor", async () => {
    ...
    (getServerSession as jest.Mock).mockResolvedValueOnce({ user: PROFESSOR_USER });
    mockPrisma.activityDay.findUnique.mockResolvedValueOnce({ id: DAY_ID, date: FUTURE_DATE });
    mockPrisma.activityDayProfessor.findFirst.mockResolvedValueOnce({ id: "adp_001" });
...
describe("DELETE /api/pickup-notices/[noticeId]", () => {
  it("returns 401 when not authenticated", async () => {
    ...
    const req = new Request(`http://localhost/api/pickup-notices/${NOTICE_ID}`, { method: "DELETE" });
...
describe("PUT /api/pickup-notices/[noticeId]", () => {
  it("returns 401 when not authenticated", async () => {
    ...
    const req = new Request(`http://localhost/api/pickup-notices/${NOTICE_ID}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ alternatePersonName: "New Name", description: "Updated" }),
    });
...
describe("POST /api/pickup-notices/[noticeId]/acknowledgments", () => {
  it("returns 401 when not authenticated", async () => {
    ...
    const req = new Request(`http://localhost/api/pickup-notices/${NOTICE_ID}/acknowledgments`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  it("idempotently updates an existing acknowledgment (upsert on repeat)", async () => {
    ...
    mockPrisma.activityDayProfessor.findFirst.mockResolvedValueOnce({ id: "adp_001" });
    mockPrisma.pickupNoticeAcknowledgment.upsert.mockResolvedValueOnce(updatedAck);
    ...
    expect(mockPrisma.pickupNoticeAcknowledgment.upsert).toHaveBeenCalledTimes(1);
  });
});
